# 🎮 Python 2D Game Editor + Engine

A simple **2D game editor** and **runtime engine** built in Python.  
Import sprites, arrange them on a scene, attach multiple Python scripts, and export a playable game with one click!

<img width="1248" height="654" alt="image" src="https://github.com/user-attachments/assets/648e55b3-231c-4619-95f1-d7381d202037" />
<img width="1248" height="660" alt="image" src="https://github.com/user-attachments/assets/a7c4995e-5cc1-4c1c-a5cf-a14b1a6df04c" />



<!-- replace with your actual GIF or screenshot -->

---

## 🚀 Features

- 🖼️ Import and manage 2D image assets  
- 🧩 Attach multiple Python scripts per object  
- 🖱️ Drag, move, and delete objects directly in the scene editor  
- 🌍 Move around and explore your scene  
- 📦 Export the scene and run it instantly with `engine.py`  
- ⚙️ Fully GUI-based editor (Tkinter)  

---

## 🧠 How It Works

1. Use the **Editor** to create and arrange your scene.  
2. Attach Python scripts (e.g. `movement.py`) to objects.  
3. Click **Export** → it generates a `scene_data` file.  
4. Run your game with:
   ```bash
   python engine.py 
